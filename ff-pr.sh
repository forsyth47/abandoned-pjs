...
segment_files=$(ls -1 file*.mp4 2>/dev/null)
if [ $? -ne 0 ]; then
...
echo "vidfile: $vidfile.mp4"


# Run FFmpeg command and capture output
# -hide_banner -loglevel quiet
ffmpeg  -hide_banner -loglevel quiet -i "$vidlink" -i "$sublink" -c:v copy -c:a copy -c:s mov_text -stats -flags +global_header -segment_time 60 $segment_start_option -f segment file%03d.mp4
